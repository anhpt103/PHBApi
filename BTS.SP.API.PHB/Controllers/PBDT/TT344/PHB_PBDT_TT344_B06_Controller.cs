...
using BTS.SP.API.PHB.ViewModels.PBDT.TT344.B06;

namespace BTS.SP.API.PHB.Controllers.PBDT
{
    [RoutePrefix("api/phb/pbdt/tt344/PHB_PBDT_TT344_B06")]
    public class PHB_PBDT_TT344_B06_Controller : ApiController
    {
        private readonly IPHB_PBDT_TT344_B06_Service _formService;
        private readonly IPHB_PBDT_TT344_B06_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_TT344_B06_DETAIL_Service _detailService;

        private readonly IAuNguoiDungService _auService;
        private readonly IDmDBHCService _dbhcService;
        private readonly IUnitOfWorkAsync _unitOfWork;

        public PHB_PBDT_TT344_B06_Controller(
            IPHB_PBDT_TT344_B06_Service service,
            IPHB_PBDT_TT344_B06_TEMPLATE_Service templateService,
            IPHB_PBDT_TT344_B06_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_TT344_B06>();
        ...
        public IHttpActionResult GetTemplate()
        {
            ...
            List<PHB_PBDT_TT344_B06_TEMPLATE> lstTemplate;
            try
            {
        ...
        public IHttpActionResult GetDetail(int formId)
        {
            ...
            try
            {
                ...
                var huyen = "";
                var xa = "";
                List<PHB_PBDT_TT344_B06_DETAIL> lstDetail;
                try
                {
                    ...
                    lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_TT344_B06_REFID == refId).OrderBy(tpl => tpl.STT_SAPXEP).ToList();
                }
                catch (Exception)
                {
                    response.Error = true;
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            // add form
            var form = new PHB_PBDT_TT344_B06
            {
                MA_DBHC = maDBHC,
                NAM = model.Form.NAM,
                NGAY_SUA = null,
            ...
            {
                detail.PHB_PBDT_TT344_B06_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream))
            {
                ...
                for (int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_TT344_B06_DETAIL
                        {
                            IS_BOLD = sheet.Cells[row, 1].Style.Font.Bold ? 1 : 0,
                            IS_ITALIC = sheet.Cells[row, 1].Style.Font.Italic ? 1 : 0,
                            PHB_PBDT_TT344_B06_REFID = form.REFID,
                            STT_SAPXEP = stt_SapXep,
                            IS_OPTIONAL = 0,

                            CHUONG = sheet.Cells[row, 1].Text,
                            LOAI = sheet.Cells[row, 2].Text,
                            KHOAN = sheet.Cells[row, 3].Text,
                            DIEN_GIAI= sheet.Cells[row, 4].Text,
                            DU_TOAN = decimal.Parse(sheet.Cells[row, 5].Value?.ToString() ?? "0"),
                        };
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_TT344_B06();
            var lstDetail = new List<PHB_PBDT_TT344_B06_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_TT344_B06_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_TT344_B06_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
