using BTS.SP.API.PHB.ViewModels.PBDT.B32;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B32")]
    public class PHB_PBDT_B32_Controller : ApiController
    {
        private readonly IPHB_PBDT_B32_Service _formService;
        private readonly IPHB_PBDT_B32_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B32_DETAIL_Service _detailService;
        ...
        public PHB_PBDT_B32_Controller(
            IPHB_PBDT_B32_Service service,
            IPHB_PBDT_B32_TEMPLATE_Service templateService,
            IPHB_PBDT_B32_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            try
            {
                ...
                var maDVQHNS = _auService.Queryable().Where(user => user.USERNAME == identity.Name).FirstOrDefault().MA_DONVI;
                lstDonVi = _dvqhnsService.Queryable().Where(dv => dv.MA_CHA == maDVQHNS || dv.MA_QHNS == maDVQHNS).ToList();
            }
            catch (Exception ex)
            {
                response.Error = true;
            ...
            {
                var lstForms = new List<PHB_PBDT_B32>();
                var lstFormViewModel = new List<FormViewModel>();

                //get list forms
                try
                {
                    lstForms = _formService.Queryable().Where(form => form.NAM_HIEN_HANH == nam && form.MA_DONVI == donVi.MA_QHNS).ToList();
                }
                catch (Exception ex)
                {
                    response.Error = true;
                ...
                if(lstFormViewModel.Count > 0)
                {
                    ...
                    {
                        MaDonVi = donVi.MA_QHNS,
                        TenDonVi = donVi.TEN_QHNS,
        ...
        {
            var response = new Response<List<PHB_PBDT_B32_TEMPLATE>>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B32_DETAIL>>();
            ...
            try
            {
                ...
                response.Data = _detailService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B32_REFID == refId)
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detail.PHB_PBDT_B32_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream)){
                ...
                for(int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B32_DETAIL
                        {
                            STT = sheet.Cells[row, 1].Value?.ToString(),
                            CHI_TIEU = sheet.Cells[row, 2].Value?.ToString(),

                            NAMHH_DU_TOAN = decimal.Parse(sheet.Cells[row, 3].Value?.ToString() ?? "0"),
                            NAMHH_UOC_THUC_HIEN = decimal.Parse(sheet.Cells[row, 4].Value?.ToString() ?? "0"),
                            NAMKH_DU_TOAN = decimal.Parse(sheet.Cells[row, 5].Value?.ToString() ?? "0"),

                            IS_BOLD = sheet.Cells[row, 2].Style.Font.Bold ? 1 : 0,
                            IS_ITALIC = sheet.Cells[row, 2].Style.Font.Italic ? 1 : 0,
                            IS_OPTIONAL = 0,
                            PHB_PBDT_B32_REFID = form.REFID,
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B32();
            var lstDetail = new List<PHB_PBDT_B32_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_B32_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_B32_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
