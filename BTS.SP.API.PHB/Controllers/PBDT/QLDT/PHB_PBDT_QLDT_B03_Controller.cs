...
using BTS.SP.API.PHB.ViewModels.PBDT.QLDT.B03;

namespace BTS.SP.API.PHB.Controllers.PBDT
{
    [RoutePrefix("api/phb/pbdt/QLDT/PHB_PBDT_QLDT_B03")]
    public class PHB_PBDT_QLDT_B03_Controller : ApiController
    {
        private readonly IPHB_PBDT_QLDT_B03_Service _formService;
        private readonly IPHB_PBDT_QLDT_B03_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_QLDT_B03_DETAIL_Service _detailService;

        private readonly IAuNguoiDungService _auService;
        private readonly IDmDBHCService _dbhcService;
        private readonly IUnitOfWorkAsync _unitOfWork;

        public PHB_PBDT_QLDT_B03_Controller(
            IPHB_PBDT_QLDT_B03_Service service,
            IPHB_PBDT_QLDT_B03_TEMPLATE_Service templateService,
            IPHB_PBDT_QLDT_B03_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_QLDT_B03>();
        ...
        public IHttpActionResult GetTemplate()
        {
            ...
            List<PHB_PBDT_QLDT_B03_TEMPLATE> lstTemplate;
            try
            {
        ...
        public IHttpActionResult GetDetail(int formId)
        {
            ...
            try
            {
                ...
                var huyen = "";
                var xa = "";
                List<PHB_PBDT_QLDT_B03_DETAIL> lstDetail;
                try
                {
                    ...
                    lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_QLDT_B03_REFID == refId).OrderBy(tpl => tpl.STT_SAPXEP).ToList();
                }
                catch (Exception)
                {
                    response.Error = true;
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            // add form
            var form = new PHB_PBDT_QLDT_B03
            {
                MA_DBHC = maDBHC,
                NAM = model.Form.NAM,
            ...
            {
                detail.PHB_PBDT_QLDT_B03_REFID = form.REFID;
        ...
        //            try
        //            {
        //                var detail = new PHB_PBDT_QLDT_B03_DETAIL
        //                {
        ...
        //                    PHB_PBDT_QLDT_B03_REFID = form.REFID,
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_QLDT_B03();
            var lstDetail = new List<PHB_PBDT_QLDT_B03_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_QLDT_B03_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
