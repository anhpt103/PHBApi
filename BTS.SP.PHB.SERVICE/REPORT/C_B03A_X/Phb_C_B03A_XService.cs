...
{
    public interface IPHB_C_B03A_XService : IBaseService<PHB_C_B03A_X>
    {
        Task<Response<C_B03A_XVm.ViewModel>> SumReport(string madbhc, string machuong, int nambc, int kybc, string lstdvqhns);
        Task<Response<C_B03A_XVm.ViewModel>> MergeReport(string madbhc, string madbhc_cha, int nambc, int kybc, List<string> changeList, string newName);
        Task<Response<C_B03A_XVm.DetailModel>> SumReport_HTML(string madbhc, string madbhc_cha, int nambc, int kybc);
    }

    public class Phb_C_B03A_XService : BaseService<PHB_C_B03A_X>, IPHB_C_B03A_XService
    {
        private readonly IRepositoryAsync<PHB_C_B03A_X> _repository;

        public Phb_C_B03A_XService(IRepositoryAsync<PHB_C_B03A_X> repository) : base(repository)
        {
            _repository = repository;
        }

        public async Task<Response<C_B03A_XVm.ViewModel>> SumReport(string madbhc, string machuong, int nambc, int kybc, string lstdvqhns)
        {
            ...
            try
            {
                using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                {
                    ...
                    using (OracleCommand command = connection.CreateCommand())
                    {
                        ...
                        command.CommandText = "PHB_C_B03A_X_SUMREPORT";
                        ...
                        {
                            List<PHB_C_B03A_X_DETAIL> lst = new List<PHB_C_B03A_X_DETAIL>();
                            while (oracleDataReader.Read())
                            {
                                lst.Add(new PHB_C_B03A_X_DETAIL()
                                {
                                    MA_CHUONG = oracleDataReader["MA_CHUONG"].ToString(),
                                    MA_TIEU_MUC = oracleDataReader["MA_TIEU_MUC"].ToString(),
                                    NOI_DUNG_THU = oracleDataReader["NOI_DUNG_THU"].ToString(),
                                    SO_TIEN = double.Parse(oracleDataReader["SO_TIEN"].ToString()),
                                    LUY_KE = double.Parse(oracleDataReader["LUY_KE"].ToString())
                                });
                            }
                            data.DETAIL = lst;
                        }
        ...
        public async Task<Response<C_B03A_XVm.DetailModel>> SumReport_HTML(string madbhc, string madbhc_cha, int nambc, int kybc)
        {
            var response = new Response<C_B03A_XVm.DetailModel>();
            ...
            try
            {
                using (var connection = new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                {
                    ...
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.CommandText = "PHB_C_B03A_X_SUMREPORT_HTML";
                        command.Parameters.Clear();
                        ...
                        var data = new C_B03A_XVm.DetailModel();
                        using (var oracleDataReader = ((OracleRefCursor)pCur.Value).GetDataReader())
                        {
                            var lst = new List<C_B03A_XVm.DetailModel.Item>();
                            while (oracleDataReader.Read())
                            {
                                int SO_TIEN, LUY_KE;
                                var isSO_TIEN = int.TryParse(oracleDataReader["SO_TIEN"].ToString(), out SO_TIEN);
                                var isLUY_KE = int.TryParse(oracleDataReader["LUY_KE"].ToString(), out LUY_KE);
                                lst.Add(new C_B03A_XVm.DetailModel.Item()
                                {
                                    NOI_DUNG_THU = oracleDataReader["NOI_DUNG_THU"].ToString(),
                                    SO_TIEN = isSO_TIEN ? SO_TIEN : 0,
                                    LUY_KE = isLUY_KE ? LUY_KE : 0
                                });
                            }
                            data.DETAIL = lst;
                        }
        ...
        public async Task<Response<C_B03A_XVm.ViewModel>> MergeReport(string madbhc, string madbhc_cha, int nambc, int kybc, List<string> changeList, string newName)
        {
            Response<C_B03A_XVm.ViewModel> response = new Response<C_B03A_XVm.ViewModel>();
            try
            {
