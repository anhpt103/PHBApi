...
namespace BTS.SP.API.PHB.Utils
{
    public class Utils
    {
        ...
        public static string GetNameMaLoaiFromCode(string code)
        {
            string result = "";
            using (var context = new STCContext())
            {
                var data = context.SYS_TUDIEN.FirstOrDefault(x => x.FIELDNAME == "MALOAI" && x.MA_TUDIEN == code);
                if (data != null) result = data.MO_TA;
            }
            return result;
        }
        ...
        public static string fomatdayphay(decimal number)
        {
            string a;
