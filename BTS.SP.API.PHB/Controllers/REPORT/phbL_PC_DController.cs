...
{
    [RoutePrefix("api/report/phbL_PC_D")]
    [Route("{id?}")]
    public class phbL_PC_DController : ApiController
    {
        private readonly IPhbL_PC_DService _L_PC_DService;
        private readonly IPhbL_PC_DDetailService _L_PC_DDetailService;
        private IUnitOfWorkAsync _unitOfWorkAsync;
        public phbL_PC_DController(IPhbL_PC_DService L_PC_UBService, IPhbL_PC_DDetailService L_PC_UBDetailService, IUnitOfWorkAsync unitOfWorkAsync)
        {
            _L_PC_DService = L_PC_UBService;
            _L_PC_DDetailService = L_PC_UBDetailService;
        ...
        public async Task<IHttpActionResult> UploadReport()
        {
            ...
            if (httpRequest.Files.Count > 0)
            {
                try
                {
                    ...
                    {
                        PHB_L_PC_D L_PC_D = new PHB_L_PC_D()
                        {
                        ...
                        if (workSheet != null)
                        {
                            ...
                            L_PC_D.MA_DBHC = httpRequest["MA_DBHC"];
                            if (string.IsNullOrEmpty(httpRequest["NAM_BC"])) return Ok(new Response()
                            {
                                Error = true,
                                Message = "Không có năm báo cáo."
                            });
                            L_PC_D.NAM_BC = int.Parse(httpRequest["NAM_BC"]);
                            if (string.IsNullOrEmpty(httpRequest["KY_BC"])) return Ok(new Response()
                            {
                                Error = true,
                                Message = "Không có kỳ báo cáo."
                            });
                            L_PC_D.KY_BC = int.Parse(httpRequest["KY_BC"]);

                            var checkReport = await _L_PC_DService.Queryable().FirstOrDefaultAsync(x =>
                                x.MA_CHUONG.Equals(L_PC_D.MA_CHUONG) && x.MA_DBHC.Equals(L_PC_D.MA_DBHC) &&
                                x.NAM_BC == L_PC_D.NAM_BC && x.KY_BC == L_PC_D.KY_BC);
                            if (checkReport != null) return Ok(new Response() { Error = true, Message = ErrorMessage.EXITS_REPORT });

                            _L_PC_DService.Insert(L_PC_D);
                            ...
                            while (workSheet.Cells[startRowTB, 1].Value != null && isLoop)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        PHB_L_PC_D_DETAIL detail = new PHB_L_PC_D_DETAIL()
                                        {
                                            ObjectState = ObjectState.Added,
                                            PHB_L_PC_UB_REFID = L_PC_D.REFID,
                                            LOAI = 1,
                                        };
                                        ...
                                        _L_PC_DDetailService.Insert(detail);
                                        startRowTB += 1;
                                    }
                                }
                            }
        ...
        [HttpPost]
        public async Task<IHttpActionResult> Post(PHB_L_PC_DVm.DTO model)
        {
            var response = new Response<string>();
            try
            {
                var data = new PHB_L_PC_D();
                //data = Mapper.Map<PHB_L_PC_DVm.DTO, PHB_L_PC_D>(model);
                var checkExist = _L_PC_DService.Queryable().FirstOrDefault(x => x.MA_DBHC.Equals(model.MA_DBHC) &&
                ...
                {
                    if (model.MA_BAO_CAO.Equals("L_PC_D"))
                    {
                    ...
                    if (!string.IsNullOrEmpty(data.MA_DBHC))
                    {
                        ...
                        _L_PC_DService.Insert(data);
                        if (model.MA_BAO_CAO == "L_PC_D")
                        {
                            if (model.DataDetails != null)
                            {
                                foreach (var bm in model.DataDetails)
                                {
                                    ...
                                    _L_PC_DDetailService.Insert(bm);
                                }
                            }
                        }
                    }
        ...
        public IHttpActionResult GetDetailByRefId(string MA_QHNS, int NAM_BC, int KY_BC)
        {
            ...
            var result = new Response<List<PHB_L_PC_D_DETAIL>>();
            List<PHB_L_PC_D_DETAIL> listResult = new List<PHB_L_PC_D_DETAIL>();
            ...
            using (OracleConnection connection = new OracleConnection(connectString))
            {
                ...
                if (connection.State == ConnectionState.Open)
                {
                    ...
                    try
                    {
                        ...
                        b.PC_KV,b.PC_CHUCVU,b.PC_THAMNIEN,b.PC_TRACHNHIEM,b.PC_CONGVU,b.PC_LOAIXA,PC_KN,b.PC_LAUNAM,b.PC_THUHUT,b.CKPT_BHXH,b.CKPT_BHYT
                        FROM PHB_L_PC_D a INNER JOIN PHB_L_PC_D_DETAIL b ON a.REFID = b.phb_l_pc_ub_refid AND a.KY_BC = b.LOAI
                        ...
                        if (dataReader.HasRows)
                        {
                            ...
                            {
                                PHB_L_PC_D_DETAIL rc = new PHB_L_PC_D_DETAIL();
                                ...
                                decimal PC_KN = 0;
                                decimal.TryParse(dataReader["PC_KN"].ToString(), out PC_KN);
                                rc.PC_KN = PC_KN;
        ...
        public IHttpActionResult GetDetailByRefIdUpdate(string refid, int KY_BC)
        {
            ...
            var response = new Response<PHB_L_PC_D_DETAIL>();
            try
            {
                Response<List<PHB_L_PC_D_DETAIL>> result = new Response<List<PHB_L_PC_D_DETAIL>>();
                var DataDetails = _L_PC_DDetailService.Queryable().Where(x => x.PHB_L_PC_UB_REFID.Equals(refid) && x.LOAI.Equals(KY_BC)).ToList();
        ...
        //[HttpPut]
        //public async Task<IHttpActionResult> Put(PHB_L_PC_DVm.EditModel model)
        //{
        ...
        //    try
        //    {
        //        PHB_L_PC_D L_PC_D = await _L_PC_DService.Queryable().FirstOrDefaultAsync(x => x.REFID.Equals(model.REFID) && x.KY_BC.Equals(model.KY_BC));
        //        if (L_PC_D == null) return Ok(new Response() { Error = true, Message = ErrorMessage.NOT_FOUND });
        //        #region Delete
        ...
        //            {
        //                PHB_L_PC_D_DETAIL item = await _L_PC_DDetailService.FindByIdAsync(itemId);
        //                if (item != null)
        //                {
        //                    item.ObjectState = ObjectState.Deleted;
        //                    _L_PC_DDetailService.Delete(item);
        //                }
        //            }
        //        }
        //        #endregion
        ...
        //                _L_PC_DDetailService.Insert(item);
        //            }
        //        }
        //        #endregion
        ...
        //            {
        //                PHB_L_PC_D_DETAIL detail = await _L_PC_DDetailService.FindByIdAsync(item.ID);
        //                if (detail != null)
        //                {
        ...
        //                    _L_PC_DDetailService.Update(detail);
        //                }
        //            }
        //        }
        //        #endregion
        ...
        //        if (hasValue)
        //        {
        //            L_PC_D.NGAY_SUA = DateTime.Now;
        //            L_PC_D.NGUOI_SUA = RequestContext.Principal.Identity.Name;
        //            _L_PC_DService.Update(L_PC_D);
        ...
        [HttpPut]
        public async Task<IHttpActionResult> Put(PHB_L_PC_DVm.DTO model)
        {
            Response<string> response = new Response<string>();
            try
            {
                var temp = _L_PC_DService.FindById(model.ID);
                if(temp != null)
                {
                    var lstDetail = _L_PC_DDetailService.Queryable().Where(x => x.PHB_L_PC_UB_REFID == temp.REFID && x.MA_BAOCAO_CHA == temp.MA_BAOCAO_CHA).ToList();
                    if (lstDetail.Count > 0)
                    {
                        ...
                        if (await _unitOfWorkAsync.SaveChangesAsync() > 0)
                        {
                            ...
                            if (model.DataDetails != null && model.DataDetails.Count > 0)
                            {
                                ...
                                {
                                    PHB_L_PC_D_DETAIL itemInsert = new PHB_L_PC_D_DETAIL();
                                    ...
                                    itemInsert.PC_KN = itemDetail.PC_KN;
                                    ...
                                    _L_PC_DDetailService.Insert(itemInsert);
                                }
                            }
                            if (await _unitOfWorkAsync.SaveChangesAsync() > 0)
                            {
