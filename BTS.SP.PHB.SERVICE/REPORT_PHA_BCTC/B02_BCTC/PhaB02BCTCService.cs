...
{
    public interface IPhaB02BCTCService : IBaseService<PHA_B02_BCTC>
    {
        ...
        string InsertData(B02BCTCModel model, PHBContext context);
    }
    public class PhaB02BCTCService : BaseService<PHA_B02_BCTC>, IPhaB02BCTCService
    {
        private readonly IRepositoryAsync<PHA_B02_BCTC> _reportRepo;
        private readonly IRepositoryAsync<PHA_B02_BCTC_TEMPLATE> _templateRepo;
        private readonly IRepositoryAsync<PHA_BCTC_B02_BCTC_TH_TEMPLATE> _bctcTemplateRepo;
        private readonly IRepositoryAsync<PHA_B02_BCTC_DETAIL> _detailRepo;

        private const string REPORT_ELEMENT_NAME = "KQHD";
        ...
        public PhaB02BCTCService(
            IRepositoryAsync<PHA_B02_BCTC> repository,
            IRepositoryAsync<PHA_B02_BCTC_TEMPLATE> templateRepo,
            IRepositoryAsync<PHA_BCTC_B02_BCTC_TH_TEMPLATE> bctcTemplateRepo,
            IRepositoryAsync<PHA_B02_BCTC_DETAIL> detailRepo) : base(repository)
        {
        ...
        public void UploadXml(XDocument document, string nguoiTao, string maDonVi, string capDuToan, int nam, DateTime ngayTao)
        {
            ...
            if (reportCount > 0)
            {
                throw new Exception($"Đã tồn tại báo cáo B02_BCTC năm {report.NAM}");
            }
            ...
            //get template
            var lstTemplate = new List<PHA_B02_BCTC_TEMPLATE>();
            try
            {
            ...
            foreach (var template in lstTemplate)
            {
                ...
                //detail
                var detail = new PHA_B02_BCTC_DETAIL
                {
                    PHA_B02_BCTC_REFID = report.REFID,
        ...
        public void ExportXml(XmlWriter writer, string maDonVi, int nam)
        {
            ...
            var report = new PHA_B02_BCTC();
            var lstTemplate = new List<PHA_BCTC_B02_BCTC_TH_TEMPLATE>();
            ...
            if (report == null)
            {
                writer.WriteStartElement("KQHDTH");
            ...
            try
            {
                using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                {
                    ...
                    command.CommandText = "BTSTC.PHA_BCTC_B02_BCTC_TH";
        ...
        {
            var exitstRp = context.PHA_B02_BCTCs.Any(rp => rp.MA_DONVI == CompanyID && rp.KY_BC == ReportPeriod && rp.NAM == ReportYear);
            if (exitstRp)
            {
                try
                {
                    var rpB02 = context.PHA_B02_BCTCs.FirstOrDefault(rp => rp.MA_DONVI == CompanyID && rp.KY_BC == ReportPeriod && rp.NAM == ReportYear);
                    rpB02.ObjectState = Repository.Pattern.Infrastructure.ObjectState.Deleted;
                    context.PHA_B02_BCTCs.Remove(rpB02);
        ...
        public string InsertData(B02BCTCModel model, PHBContext context)
        {
            if (model == null) return "B02BCTCModel model is null";

            var rpB02_BCTC = new PHA_B02_BCTC
            {
            ...
            try
            {
                context.PHA_B02_BCTCs.Add(rpB02_BCTC);

                List<PHA_B02_BCTC_DETAIL> lstRpB02_BCTC_DETAIL = new List<PHA_B02_BCTC_DETAIL>();

                foreach (var rpDt in model.B02BCTCDetail)
                {
                    var rpBCTC_DETAIL = new PHA_B02_BCTC_DETAIL
                    {
                        PHA_B02_BCTC_REFID = model.ReportHeader.RefID,
                        STT_SAPXEP = rpDt.ReportItemIndex,
                        CHI_TIEU = rpDt.ReportItemName,
                        STT = rpDt.ReportItemAlias,
                        MA_SO = rpDt.ReportItemCode,
                        SO_NAM_NAY = rpDt.CurrentPeriodAmount,
                        SO_NAM_TRUOC = rpDt.PreviousPeriodAmount,
                        IS_BOLD = 0,
                        IS_ITALIC = 0,
                        THUYET_MINH = rpDt.ReportItemDescription,
                        ObjectState = Repository.Pattern.Infrastructure.ObjectState.Added
                    };
                    lstRpB02_BCTC_DETAIL.Add(rpBCTC_DETAIL);
                }

                if (lstRpB02_BCTC_DETAIL.Count == 0) return "lstRpB02_BCTC_DETAIL trong PhaB02BCTCService.InsertData có độ dài = 0";
                context.PHA_B02_BCTC_DETAILs.AddRange(lstRpB02_BCTC_DETAIL);
            }
            catch (Exception ex)
            {
                return ex.Message;
