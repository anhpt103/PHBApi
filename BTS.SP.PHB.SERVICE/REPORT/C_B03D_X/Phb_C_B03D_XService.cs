...
{
    public interface IPhb_C_B03D_XService : IBaseService<PHB_C_B03D_X>
    {
        Task<Response<C_B03B_XVm.ViewModel>> SumReport(string machuong, int nambc, int kybc, string lstdvqhns);
        Task<Response<C_B03d_XVm.ViewModel>> MergeReport(string madbhc, string madbhc_cha, int nambc, int kybc, List<string> changeList, string newName);
        Task<Response<C_B03d_XVm.DetailModel>> SumReport_HTML(string madbhc, string madbhc_cha, int nambc, int kybc);
    }

    public class Phb_C_B03D_XService : BaseService<PHB_C_B03D_X>, IPhb_C_B03D_XService
    {
        private readonly IRepositoryAsync<PHB_C_B03D_X> _repository;

        public Phb_C_B03D_XService(IRepositoryAsync<PHB_C_B03D_X> repository) : base(repository)
        {
            _repository = repository;
        }

        public async Task<Response<C_B03B_XVm.ViewModel>> SumReport(string machuong, int nambc, int kybc, string lstdvqhns)
        {
        ...
        public async Task<Response<C_B03d_XVm.DetailModel>> SumReport_HTML(string madbhc, string madbhc_cha, int nambc, int kybc)
        {
            var response = new Response<C_B03d_XVm.DetailModel>();
            ...
            try
            {
                using (var connection = new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                {
                    ...
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.CommandText = "PHB_C_B03D_X_SUMREPORT_HTML";
                        command.Parameters.Clear();
                        ...
                        var data = new C_B03d_XVm.DetailModel();
                        using (var oracleDataReader = ((OracleRefCursor)pCur.Value).GetDataReader())
                        {
                            var lst = new List<C_B03d_XVm.DetailModel.Item>();
                            while (oracleDataReader.Read())
                            {
                                int DUTOANNAM, QUYETTOANNAM;
                                var isDUTOANNAM = int.TryParse(oracleDataReader["DUTOANNAM"].ToString(), out DUTOANNAM);
                                var isQUYETTOANNAM = int.TryParse(oracleDataReader["QUYETTOANNAM"].ToString(), out QUYETTOANNAM);
                                lst.Add(new C_B03d_XVm.DetailModel.Item()
                                {
                                    TENCHITIEU = oracleDataReader["TENCHITIEU"].ToString(),
                                    DUTOANNAM = isDUTOANNAM ? DUTOANNAM : 0,
                                    QUYETTOANNAM = isQUYETTOANNAM ? QUYETTOANNAM : 0
                                });
                            }
                            data.DETAIL = lst;
                        }
        ...
        public async Task<Response<C_B03d_XVm.ViewModel>> MergeReport(string madbhc, string madbhc_cha, int nambc, int kybc, List<string> changeList, string newName)
        {
            Response<C_B03d_XVm.ViewModel> response = new Response<C_B03d_XVm.ViewModel>();
            try
            {
                    ...
                    new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                {
                    ...
                    {
                        command.CommandType = CommandType.StoredProcedure;
                        command.CommandText = "PHB_C_B03A_X_MERGEREPORT";
                        command.Parameters.Clear();
                        var pmadbhc = command.Parameters.Add("MADBHC", OracleDbType.NVarchar2);
                        pmadbhc.Direction = ParameterDirection.Input;
                        pmadbhc.Size = 5;
                        ...
                        C_B03d_XVm.ViewModel data = new C_B03d_XVm.ViewModel();
                        using (OracleDataReader oracleDataReader = ((OracleRefCursor)pCur.Value).GetDataReader())
                        {
                            List<PHB_C_B03D_X_DETAIL> lst = new List<PHB_C_B03D_X_DETAIL>();
                            while (oracleDataReader.Read())
                            {
                                lst.Add(new PHB_C_B03D_X_DETAIL()
                                {
