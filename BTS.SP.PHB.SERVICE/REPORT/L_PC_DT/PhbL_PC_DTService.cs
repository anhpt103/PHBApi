...
{
    public interface IPhbL_PC_DTService : IBaseService<PHB_L_PC_DT>
    {
        Task<Response<PHB_L_PC_DTVm.ViewModel>> SumReport(string machuong, int nambc, int kybc, string lstdvqhns);
    }
    public class PhbL_PC_DTService : BaseService<PHB_L_PC_DT>, IPhbL_PC_DTService
    {
        private readonly IRepositoryAsync<PHB_L_PC_DT> _repository;
        public PhbL_PC_DTService(IRepositoryAsync<PHB_L_PC_DT> repository) : base(repository)
        {
            _repository = repository;
        }

        public async Task<Response<PHB_L_PC_DTVm.ViewModel>> SumReport(string machuong, int nambc, int kybc, string lstdvqhns)
        {
            Response<PHB_L_PC_DTVm.ViewModel> response = new Response<PHB_L_PC_DTVm.ViewModel>();
            try
            {
                    ...
                    new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                {
                    ...
                    using (OracleCommand command = connection.CreateCommand())
                    {
                        ...
                        command.CommandText = "PHB_L_PC_DT_SUMREPORT";
                        ...
                        PHB_L_PC_DTVm.ViewModel data = new PHB_L_PC_DTVm.ViewModel();
                        using (OracleDataReader oracleDataReader = ((OracleRefCursor)pCur.Value).GetDataReader())
                        {
                            List<PHB_L_PC_DT_DETAIL> lstTrongBang = new List<PHB_L_PC_DT_DETAIL>();
                            List<PHB_L_PC_DT_DETAIL> lstNgoaiBang = new List<PHB_L_PC_DT_DETAIL>();
                            while (oracleDataReader.Read())
                            {
                                int loai = int.Parse(oracleDataReader["LOAI"].ToString());
                                if (loai == 1)
                                {
                                    lstTrongBang.Add(new PHB_L_PC_DT_DETAIL()
                                    {
                                ...
                                {
                                    lstNgoaiBang.Add(new PHB_L_PC_DT_DETAIL()
                                    {
                                        STT = oracleDataReader["STT"].ToString(),
                                        HO_VATEN = oracleDataReader["HO_VATEN"].ToString(),
                                        CHUC_DANH = oracleDataReader["CHUC_DANH"].ToString(),
