...
using BTS.SP.API.PHB.ViewModels.PBDT.B121;
using OfficeOpenXml;
using Repository.Pattern.Infrastructure;
using BTS.SP.PHB.SERVICE.SYS.SysDvqhns_QuanLy;
using BTS.SP.PHB.ENTITY.Sys;
using BTS.SP.API.PHB.ViewModels.PBDT.TT342.B121;

namespace BTS.SP.API.PHB.Controllers.PBDT
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B121")]
    public class PHB_PBDT_B121_Controller : ApiController
    {
        private readonly IPHB_PBDT_B121_Serviec _b121Service;
        private readonly IPHB_PBDT_B121_DETAIL_Service _detailService;
        private readonly IPHB_PBDT_B121_TEMPLATE_Service _templateService;
        ...
        public PHB_PBDT_B121_Controller(
            IPHB_PBDT_B121_Serviec service,
            IPHB_PBDT_B121_DETAIL_Service detailService,
            IPHB_PBDT_B121_TEMPLATE_Service templateService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B121>();
                var lstFormViewModel = new List<FormViewModel>();

                try
                {                    
                     lstForms = _b121Service.Queryable().Where(x => x.NAM_HIEN_HANH == nam && x.MA_DONVI == donVi.MA_DVQHNS).ToList();                  
                }
                catch (Exception ex)
                {
                    response.Error = true;
        ...
        {
            var response = new Response<List<PHB_PBDT_B121_TEMPLATE>>();
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            var formCount = _b121Service.Queryable().Where(x => x.MA_DONVI == form.MA_DONVI && x.NAM_HIEN_HANH == form.NAM_HIEN_HANH ).Count();
            ...
            _b121Service.Insert(form);

            foreach (var detail in model.Details)
            {
                detail.PHB_PBDT_B121_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            var formCount = _b121Service.Queryable()
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream))
            {
                ...
                for (int row = startRow; row <= endRow; row ++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B121_DETAIL
                        {
                            ...
                            PHB_PBDT_B121_REFID = form.REFID,
        ...
        {
            var response = new Response<List<PHB_PBDT_B121_DETAIL>>();

            try
            {
                var form = _b121Service.Queryable().Where(x => x.ID == id).FirstOrDefault();
                ...
                response.Data = _detailService.Queryable().Where(x => x.PHB_PBDT_B121_REFID == form.REFID).OrderBy(x => x.STT_SAPXEP).ToList();
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B121();
            var lstDetail = new List<PHB_PBDT_B121_DETAIL>();

            try
            {
                form = _b121Service.Queryable().Where(x => x.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(x => x.PHB_PBDT_B121_REFID == form.REFID).ToList();
            }
            catch(Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        public IHttpActionResult Delete(int id)
        {
            ...
            try
            {
                form = _b121Service.Queryable().FirstOrDefault(x => x.ID == id);
            }
            catch(Exception ex)
            {
                WriteLogs.LogError(ex);
