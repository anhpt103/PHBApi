...
namespace BTS.SP.PHB.SERVICE.AUTH.Shared
{
    public interface ISharedService:IBaseService<BaseEntity>
    {
        Task<RoleState> GetRoleStateByMaChucNang(string phanhe,string username,string machucnang);
    }
    public class SharedService: BaseService<BaseEntity>,ISharedService
    {
        private readonly IRepositoryAsync<BaseEntity> _repository;
        public SharedService(IRepositoryAsync<BaseEntity> repository) : base(repository)
        {
