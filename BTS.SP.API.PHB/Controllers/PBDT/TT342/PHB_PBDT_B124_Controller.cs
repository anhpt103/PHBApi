using BTS.SP.API.PHB.ViewModels.PBDT.B124;
using BTS.SP.API.PHB.ViewModels.PBDT.Main;
using BTS.SP.API.PHB.ViewModels.PBDT.TT342.B124;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B124")]
    public class PHB_PBDT_B124_Controller : ApiController
    {
        private readonly IPHB_PBDT_B124_Service _B124Service;
        private readonly IPHB_PBDT_B124_DETAIL_Service _detailService;
        private readonly IPHB_PBDT_B124_TEMPLATE_Service _templateService;
        ...
        public PHB_PBDT_B124_Controller(
           IPHB_PBDT_B124_Service service,
           IPHB_PBDT_B124_DETAIL_Service detailService,
           IPHB_PBDT_B124_TEMPLATE_Service templateService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B124>();
                var lstFormViewModel = new List<FormViewModel>();

                try
                {
                    lstForms = _B124Service.Queryable().Where(x => x.NAM_HIEN_HANH == nam && x.MA_DONVI == donVi.MA_DVQHNS).ToList();
                }
                catch (Exception ex)
                {
                    response.Error = true;
        ...
        {
            var response = new Response<List<PHB_PBDT_B124_TEMPLATE>>();
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            if (model == null || model.forms == null || model.Details.Count == 0)
            {
                response.Error = true;
                response.Message = ErrorMessage.EMPTY_DATA;
                return Ok(response);
            ...
            var form = new PHB_PBDT_B124
            {
                ...
                NGUOI_SUA = null,
                NGAY_SUA = null,
                MA_DONVI = model.forms.MA_DONVI,
                DON_VI_DT = donvidutoan,
                CAP_DU_TOAN = capdutoan,
                NAM_HIEN_HANH = model.forms.NAM_HIEN_HANH,
                NAM_KE_HOACH = model.forms.NAM_HIEN_HANH + 1,
                NAM_THUC_HIEN = model.forms.NAM_THUC_HIEN - 1,
                CHUONG = chuong,
                
            };

            var formCount = _B124Service.Queryable().Where(x => x.MA_DONVI == form.MA_DONVI && x.NAM_HIEN_HANH == form.NAM_HIEN_HANH).Count();
            ...
            _B124Service.Insert(form);

            foreach(var detail in model.Details)
            {
                detail.PHB_PBDT_B124_REFID = form.REFID;
        ...
        {
            var response = new Response<List<PHB_PBDT_B124_DETAIL>>();
            var form = new PHB_PBDT_B124();

            try
            {
                form = _B124Service.Queryable().FirstOrDefault(x => x.ID.Equals(id));
                response.Data = _detailService.Queryable().Where(x => x.PHB_PBDT_B124_REFID.Equals(form.REFID)).OrderBy(x => x.STT_SAPXEP).ToList();
            }
            catch(Exception ex)
            {
                response.Error = true;
        ...
        public IHttpActionResult Edit(Edit_viewModel model)
        {
            var response = new Response();
            var form = new PHB_PBDT_B124();
            var lstDetail = new List<PHB_PBDT_B124_DETAIL>();

            try
            {
                
                form = _B124Service.Queryable().FirstOrDefault(x => x.ID.Equals(model.FormId));
                lstDetail = _detailService.Queryable().Where(x => x.PHB_PBDT_B124_REFID.Equals(form.REFID)).ToList();
            }
            catch(Exception ex)
            {
                response.Error = true;
            ...
            try
            {
                ...
                {
                    details.ID = new int();
                    details.PHB_PBDT_B124_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            var formCount = _B124Service.Queryable()
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream))
            {
                ...
                for (int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B124_DETAIL
                        {
                            ...
                            PHB_PBDT_B124_REFID = form.REFID,
        ...
        public IHttpActionResult Delete(int id)
        {
            ...
            {
                form = _B124Service.Queryable().FirstOrDefault(x => x.ID.Equals(id));
                lstDetail = _detailService.Queryable().Where(x => x.PHB_PBDT_B124_REFID == form.REFID).ToList();
            }
            catch(Exception ex)
            {
                response.Error = true;
            ...
            _B124Service.Delete(form);

            foreach(var details in lstDetail)
            {
                _detailService.Delete(details);
            }
        ...
        public IHttpActionResult Approve(int formId)
        {
            ...
            try
            {
                form = _B124Service.Queryable().Where(frm => frm.ID == formId).FirstOrDefault();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
