using BTS.SP.API.PHB.ViewModels.PBDT.B111;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B111")]
    public class PHB_PBDT_B111_Controller : ApiController
    {
        private readonly IPHB_PBDT_B111_Service _formService;
        private readonly IPHB_PBDT_B111_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B111_DETAIL_Service _detailService;
        ...
        public PHB_PBDT_B111_Controller(
            IPHB_PBDT_B111_Service service,
            IPHB_PBDT_B111_TEMPLATE_Service templateService,
            IPHB_PBDT_B111_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B111>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B111_TEMPLATE>>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B111_DETAIL>>();
            ...
            try
            {
                ...
                response.Data = _detailService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B111_REFID == refId)
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detail.PHB_PBDT_B111_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream))
            {
                ...
                for (int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B111_DETAIL
                        {
                            ...
                            PHB_PBDT_B111_REFID = form.REFID,
                            STT_SAPXEP = stt_SapXep,
                            IS_OPTIONAL = 0,

                            NAMHH_DT_TONG_SO = decimal.Parse(sheet.Cells[row, 3].Value?.ToString() ?? "0"),
                            NAMHH_DT_CHI_DTPT = decimal.Parse(sheet.Cells[row, 4].Value?.ToString() ?? "0"),
                            NAMHH_DT_CHI_THUONGXUYEN= decimal.Parse(sheet.Cells[row, 5].Value?.ToString() ?? "0"),

                            NAMHH_UOCTH_TONG_SO = decimal.Parse(sheet.Cells[row, 6].Value?.ToString() ?? "0"),
                            NAMHH_UOCTH_CHI_DTPT = decimal.Parse(sheet.Cells[row, 7].Value?.ToString() ?? "0"),
                            NAMHH_UOCTH_CHI_THUONGXUYEN = decimal.Parse(sheet.Cells[row, 8].Value?.ToString() ?? "0"),

                            NAMKH_DT_TONG_SO = decimal.Parse(sheet.Cells[row, 9].Value?.ToString() ?? "0"),
                            NAMKH_DT_CHI_DTPT = decimal.Parse(sheet.Cells[row, 10].Value?.ToString() ?? "0"),
                            NAMKH_DT_CHI_THUONG_XUYEN = decimal.Parse(sheet.Cells[row, 11].Value?.ToString() ?? "0"),
                        };
                        ...
                        {
                            response.Error = true;
                            response.Message = $"Lỗi dữ liệu: Tổng số không bằng tổng Chi ĐTPT và Chi thường xuyên (dòng {row})";
                            return Ok(response);
                        }
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B111();
            var lstDetail = new List<PHB_PBDT_B111_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_B111_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_B111_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
        ...
        private bool CheckSum(PHB_PBDT_B111_DETAIL detail)
        {
            if(detail.NAMHH_DT_TONG_SO != detail.NAMHH_DT_CHI_DTPT + detail.NAMHH_DT_CHI_THUONGXUYEN)
            {
                return false;
            }

            if (detail.NAMHH_UOCTH_TONG_SO != detail.NAMHH_UOCTH_CHI_DTPT + detail.NAMHH_UOCTH_CHI_THUONGXUYEN)
            {
                return false;
            }

            if (detail.NAMKH_DT_TONG_SO != detail.NAMKH_DT_CHI_DTPT + detail.NAMKH_DT_CHI_THUONG_XUYEN)
            {
                return false;
            }
