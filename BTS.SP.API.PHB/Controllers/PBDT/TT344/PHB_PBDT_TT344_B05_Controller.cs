...
using BTS.SP.API.PHB.ViewModels.PBDT.TT344.B05;

namespace BTS.SP.API.PHB.Controllers.PBDT
{
    [RoutePrefix("api/phb/pbdt/tt344/PHB_PBDT_TT344_B05")]
    public class PHB_PBDT_TT344_B05_Controller : ApiController
    {
        private readonly IPHB_PBDT_TT344_B05_Service _formService;
        private readonly IPHB_PBDT_TT344_B05_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_TT344_B05_DETAIL_Service _detailService;

        private readonly IAuNguoiDungService _auService;
        private readonly IDmDBHCService _dbhcService;
        private readonly IUnitOfWorkAsync _unitOfWork;

        public PHB_PBDT_TT344_B05_Controller(
            IPHB_PBDT_TT344_B05_Service service,
            IPHB_PBDT_TT344_B05_TEMPLATE_Service templateService,
            IPHB_PBDT_TT344_B05_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_TT344_B05>();
        ...
        public IHttpActionResult GetTemplate()
        {
            ...
            List<PHB_PBDT_TT344_B05_TEMPLATE> lstTemplate;
            try
            {
        ...
        public IHttpActionResult GetDetail(int formId)
        {
            ...
            try
            {
                ...
                var huyen = "";
                var xa = "";
                List<PHB_PBDT_TT344_B05_DETAIL> lstDetail;
                try
                {
                    ...
                    lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_TT344_B05_REFID == refId).OrderBy(tpl => tpl.STT_SAPXEP).ToList();
                }
                catch (Exception)
                {
                    response.Error = true;
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            // add form
            var form = new PHB_PBDT_TT344_B05
            {
                MA_DBHC = maDBHC,
                NAM = model.Form.NAM,
                NGAY_SUA = null,
            ...
            {
                detail.PHB_PBDT_TT344_B05_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream))
            {
                ...
                for (int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_TT344_B05_DETAIL
                        {
                            ...
                            PHB_PBDT_TT344_B05_REFID = form.REFID,
                            STT_SAPXEP = stt_SapXep,
                            IS_OPTIONAL = 0,

                            UOC_TH_THU = decimal.Parse(sheet.Cells[row, 3].Value?.ToString() ?? "0"),
                            UOC_TH_CHI = decimal.Parse(sheet.Cells[row, 4].Value?.ToString() ?? "0"),
                            UOC_TH_CHENH_LECH = decimal.Parse(sheet.Cells[row, 5].Value?.ToString() ?? "0"),
                            KEHOACH_THU = decimal.Parse(sheet.Cells[row, 6].Value?.ToString() ?? "0"),
                            KEHOACH_CHI = decimal.Parse(sheet.Cells[row, 7].Value?.ToString() ?? "0"),
                            KEHOACH_CHENH_LECH = decimal.Parse(sheet.Cells[row, 8].Value?.ToString() ?? "0"),
                        };
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_TT344_B05();
            var lstDetail = new List<PHB_PBDT_TT344_B05_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_TT344_B05_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_TT344_B05_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
