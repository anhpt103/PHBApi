using BTS.SP.API.PHB.ViewModels.PBDT.B1311;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B1311")]
    public class PHB_PBDT_B1311_Controller : ApiController
    {
        private readonly IPHB_PBDT_B1311_Service _formService;
        private readonly IPHB_PBDT_B1311_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B1311_DETAIL_Service _detailService;
        ...
        public PHB_PBDT_B1311_Controller(
            IPHB_PBDT_B1311_Service service,
            IPHB_PBDT_B1311_TEMPLATE_Service templateService,
            IPHB_PBDT_B1311_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B1311>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1311_TEMPLATE>>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1311_DETAIL>>();
            ...
            try
            {
                ...
                response.Data = _detailService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B1311_REFID == refId)
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detail.PHB_PBDT_B1311_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream)){
                ...
                for(int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B1311_DETAIL
                        {
                            STT = sheet.Cells[row, 1].Value?.ToString(),
                            CHI_TIEU = sheet.Cells[row, 2].Value?.ToString(),

                            DOITUONG_NAMTRUOC = int.Parse(sheet.Cells[row, 3].Value?.ToString() ?? "0"),
                            DOITUONG_NAMHH = int.Parse(sheet.Cells[row, 4].Value?.ToString() ?? "0"),
                            DOITUONG_NAMKH = int.Parse(sheet.Cells[row, 5].Value?.ToString() ?? "0"),
                            DOITUONG_TANG_GIAM = int.Parse(sheet.Cells[row, 6].Value?.ToString() ?? "0"),
                            DOITUONG_TY_LE = double.Parse(sheet.Cells[row, 7].Value?.ToString() ?? "0"),

                            SOTIEN_NAMTRUOC = decimal.Parse(sheet.Cells[row, 8].Value?.ToString() ?? "0"),
                            SOTIEN_NAMHH_DT = decimal.Parse(sheet.Cells[row, 9].Value?.ToString() ?? "0"),
                            SOTIEN_NAMHH_UOCTHUCHIEN = decimal.Parse(sheet.Cells[row, 10].Value?.ToString() ?? "0"),
                            SOTIEN_NAMKH = decimal.Parse(sheet.Cells[row, 11].Value?.ToString() ?? "0"),
                            SOTIEN_TANG_GIAM = decimal.Parse(sheet.Cells[row, 12].Value?.ToString() ?? "0"),
                            SOTIEN_TY_LE = double.Parse(sheet.Cells[row, 13].Value?.ToString() ?? "0"),

                            IS_BOLD = sheet.Cells[row, 2].Style.Font.Bold ? 1 : 0,
                            IS_ITALIC = sheet.Cells[row, 2].Style.Font.Italic ? 1 : 0,
                            IS_OPTIONAL = 0,
                            PHB_PBDT_B1311_REFID = form.REFID,
                        ...
                        {
                            response.Error = true;
                            response.Message = $"Lỗi dữ liệu: Tăng/giảm dự toán không bằng hiệu các thành phần (dòng số {row})";
                            return Ok(response);
                        }
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B1311();
            var lstDetail = new List<PHB_PBDT_B1311_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_B1311_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_B1311_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
        ...
        private bool CheckSum(PHB_PBDT_B1311_DETAIL detail)
        {
            if (detail.DOITUONG_TANG_GIAM != detail.DOITUONG_NAMKH - detail.DOITUONG_NAMHH)
            {
                return false;
            }

            if (detail.SOTIEN_TANG_GIAM != detail.SOTIEN_NAMKH - detail.SOTIEN_NAMHH_UOCTHUCHIEN)
            {
                return false;
            }
