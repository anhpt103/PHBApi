...
{
    public interface IPhbL_PC_UBService : IBaseService<PHB_L_PC_UB>
    {
        Task<Response<PHB_L_PC_UBVm.ViewModel>> SumReport(string machuong, int nambc, int kybc, string lstdvqhns);
    }
    public class PhbL_PC_UBService : BaseService<PHB_L_PC_UB>, IPhbL_PC_UBService
    {
        private readonly IRepositoryAsync<PHB_L_PC_UB> _repository;
        public PhbL_PC_UBService(IRepositoryAsync<PHB_L_PC_UB> repository) : base(repository)
        {
            _repository = repository;
        }

        public async Task<Response<PHB_L_PC_UBVm.ViewModel>> SumReport(string machuong, int nambc, int kybc, string lstdvqhns)
        {
            Response<PHB_L_PC_UBVm.ViewModel> response = new Response<PHB_L_PC_UBVm.ViewModel>();
            try
            {
                    ...
                    new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                {
                    ...
                    using (OracleCommand command = connection.CreateCommand())
                    {
                        ...
                        PHB_L_PC_UBVm.ViewModel data = new PHB_L_PC_UBVm.ViewModel();
                        using (OracleDataReader oracleDataReader = ((OracleRefCursor)pCur.Value).GetDataReader())
                        {
                            List<PHB_L_PC_UB_DETAIL> lstTrongBang = new List<PHB_L_PC_UB_DETAIL>();
                            List<PHB_L_PC_UB_DETAIL> lstNgoaiBang = new List<PHB_L_PC_UB_DETAIL>();
                            while (oracleDataReader.Read())
                            {
                                int loai = int.Parse(oracleDataReader["LOAI"].ToString());
                                if (loai == 1)
                                {
                                    lstTrongBang.Add(new PHB_L_PC_UB_DETAIL()
                                    {
                                ...
                                {
                                    lstNgoaiBang.Add(new PHB_L_PC_UB_DETAIL()
                                    {
                                        STT = oracleDataReader["STT"].ToString(),
                                        HO_VATEN = oracleDataReader["HO_VATEN"].ToString(),
                                        CHUC_DANH = oracleDataReader["CHUC_DANH"].ToString(),
