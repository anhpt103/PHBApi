...
{
    [RoutePrefix("api/PhuLuc/pl01_TT137")]
    [Route("{id?}")]
    public class PhbPL01TT137Controller : ApiController
    {
        private readonly IPHB_PL01_TT137Service _service;
        private readonly IPHB_PL01_TT137DetailService _serviceDetail;
        private readonly IUnitOfWorkAsync _unitOfWorkAsync;
        private readonly ISysDvqhnsService _sysDVQHNSService;

        public PhbPL01TT137Controller(IPHB_PL01_TT137Service service, IPHB_PL01_TT137DetailService serviceDetail, IUnitOfWorkAsync unitOfWorkAsync, ISysDvqhnsService sysDvqhnsService)
        {
            _service = service;
        ...
        public async Task<IHttpActionResult> AddNew(PL01_TT137ViewModel.AddModel model)
        {
            ...
            var dataDetail = new PHB_PL01_TT137_DETAIL
            {
                PHB_PL01_TT137_REFID = data.REFID,
        ...
        [Route("Update")]
        [HttpPost]
        public async Task<IHttpActionResult> Update(PL01_TT137ViewModel.EditModel model)
        {
            var response = new Response();
            var identity = (ClaimsIdentity)HttpContext.Current.User.Identity;

            var data = _service.Queryable().FirstOrDefault(x => x.REFID == model.data.REFID);
            var dataDetail = _serviceDetail.Queryable().FirstOrDefault(x => x.PHB_PL01_TT137_REFID == data.REFID);
            if (data != null)
            {
            ...
            if (dataDetail != null)
            {
                ...
                model.dataDetail.PHB_PL01_TT137_REFID = model.data.REFID;
                try
                {
                    using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["STCConnection"].ConnectionString))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                command.CommandText = "DELETE FROM PHB_" + model.MA_BAO_CAO + "_DETAIL WHERE PHB_" + model.MA_BAO_CAO + "_REFID='" + model.dataDetail.PHB_PL01_TT137_REFID + "'";
            ...
            {
                dataDetail.PHB_PL01_TT137_REFID = data.REFID;
        ...
        {
            Response<PHB_PL01_TT137_DETAIL> response = new Response<PHB_PL01_TT137_DETAIL>();
            ...
            else
            {
                try
                {
                    ...
                    var objDetail = _serviceDetail.Queryable().FirstOrDefault(x => x.PHB_PL01_TT137_REFID == obj.REFID);
        ...
        {
            Response<PHB_PL01_TT137> response = new Response<PHB_PL01_TT137>();
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
        ...
        [Route("PHB_PL_TT137_SUMREPORT")]
        [HttpPost]
        public async Task<IHttpActionResult> PHB_PL_TT137_SUMREPORT(PL01_TT137ReportViewModel model)
        {
            var response = new Response<PL01_TT137ReportViewModel>();
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
        ...
        private PL01_TT137ReportViewModel LoadStoredPHB_B01A_TT137_SUMREPORT(PL01_TT137ReportViewModel model)
        {
            var obj = new PL01_TT137ReportViewModel();
            try
            {
