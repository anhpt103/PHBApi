...
using BTS.SP.API.PHB.ViewModels.PBDT.B1501;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B1501")]
    public class PHB_PBDT_B1501_Controller : ApiController
    {
        private readonly IPHB_PBDT_B1501_Service _formService;
        private readonly IPHB_PBDT_B1501_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B1501_ROW_Service _rowService;
        private readonly IPHB_PBDT_B1501_COLUMN_Service _colService;
        private readonly IPHB_PBDT_B1501_DATA_Service _dataService;
        ...
        public PHB_PBDT_B1501_Controller(
            IPHB_PBDT_B1501_Service service,
            IPHB_PBDT_B1501_TEMPLATE_Service templateService,
            IPHB_PBDT_B1501_ROW_Service rowService,
            IPHB_PBDT_B1501_COLUMN_Service colService,
            IPHB_PBDT_B1501_DATA_Service dataService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B1501>();
        ...
        public IHttpActionResult GetTemplate()
        {
            ...
            try
            {
                ...
                {
                    var row = new PHB_PBDT_B1501_ROW
                    {
                    ...
                    var lstData = new List<PHB_PBDT_B1501_DATA>();

                    foreach(var col in lstCol)
                    {
                        var data = new PHB_PBDT_B1501_DATA
                        {
        ...
        public IHttpActionResult GetDetail(int formId)
        {
            ...
            try
            {
                ...
                var lstRow = _rowService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B1501_REFID == refId)
                ...
                foreach(var row in lstRow)
                {
                    ...
                    {
                        Row = row,
                        Datas = new List<PHB_PBDT_B1501_DATA>()
                    };
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detailVm.Row.PHB_PBDT_B1501_REFID = form.REFID;
                ...
                while (sheet.Cells[startRow, 2].Value?.ToString() != null)
                {
                    try
                    {
                        ...
                        var row = new PHB_PBDT_B1501_ROW
                        {
                            ...
                            PHB_PBDT_B1501_REFID = form.REFID,
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B1501();
            var lstRow = new List<PHB_PBDT_B1501_ROW>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstRow = _rowService.Queryable().Where(detail => detail.PHB_PBDT_B1501_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        public IHttpActionResult Delete(int formId)
        {
            ...
            try
            {
                var lstRow = _rowService.Queryable().Where(detail => detail.PHB_PBDT_B1501_REFID == form.REFID).ToList();
                //delete list details
                foreach (var row in lstRow)
                {
        ...
        private bool IsOptional(List<PHB_PBDT_B1501_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
