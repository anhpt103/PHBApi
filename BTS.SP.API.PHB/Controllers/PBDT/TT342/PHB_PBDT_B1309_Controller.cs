using BTS.SP.API.PHB.ViewModels.PBDT.B1309;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B1309")]
    public class PHB_PBDT_B1309_Controller : ApiController
    {
        private readonly IPHB_PBDT_B1309_Service _formService;
        private readonly IPHB_PBDT_B1309_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B1309_DETAIL_Service _detailService;
        ...
        public PHB_PBDT_B1309_Controller(
            IPHB_PBDT_B1309_Service service,
            IPHB_PBDT_B1309_TEMPLATE_Service templateService,
            IPHB_PBDT_B1309_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B1309>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1309_TEMPLATE>>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1309_DETAIL>>();
            ...
            try
            {
                ...
                response.Data = _detailService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B1309_REFID == refId)
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detail.PHB_PBDT_B1309_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream)){
                ...
                for(int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B1309_DETAIL
                        {
                            ...
                            LUY_KE = decimal.Parse(sheet.Cells[row, 6].Value?.ToString() ?? "0"),
                            DU_TOAN = decimal.Parse(sheet.Cells[row, 7].Value?.ToString() ?? "0"),

                            IS_BOLD = sheet.Cells[row, 2].Style.Font.Bold ? 1 : 0,
                            IS_ITALIC = sheet.Cells[row, 2].Style.Font.Italic ? 1 : 0,
                            IS_OPTIONAL = 0,
                            PHB_PBDT_B1309_REFID = form.REFID,
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B1309();
            var lstDetail = new List<PHB_PBDT_B1309_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_B1309_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_B1309_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
