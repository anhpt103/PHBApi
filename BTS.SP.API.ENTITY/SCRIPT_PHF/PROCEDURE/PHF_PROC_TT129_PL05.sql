create or replace PROCEDURE PHF_PROC_TT129_PL05
(
  P_TUNGAY IN DATE,
  P_DENNGAY IN DATE,
...
BEGIN
    ...
    BEGIN
        ...
        INNER JOIN PHF_TT129_PL05 b ON a.MA_FILE_PK = b.MA_FILE_PK 
        INNER JOIN PHF_TT129_PL05_TEMPLATE c ON b.MADONG = c.MADONG
        ...
        AND LENGTH(c.I_STATE) = 8 ORDER BY c.I_STATE';
        OPEN CURSOR_CONTENT FOR QUERRY_CONTENT;
            LOOP
               FETCH CURSOR_CONTENT INTO T_NOIDUNG,T_I_STATE;
               EXIT WHEN CURSOR_CONTENT%NOTFOUND;
               QUERRY_DYNAMIC_PIVOT := QUERRY_DYNAMIC_PIVOT || '''' || T_NOIDUNG || '''' || ' AS ' || T_I_STATE || ',';
               
            END LOOP;
            QUERRY_DYNAMIC_PIVOT := SUBSTR(QUERRY_DYNAMIC_PIVOT,0, LENGTH(QUERRY_DYNAMIC_PIVOT) - 1);
        CLOSE CURSOR_CONTENT;
--         DBMS_OUTPUT.put_line(QUERRY_CONTENT);
    END;
   
    QUERRY_SELECT_PIVOT := 'SELECT TEN,SUM(PARENT_A) AS TIEUCHI_A,SUM(PARENT_B) AS TIEUCHI_B,SUM(PARENT_C) AS TIEUCHI_C
FROM
    (
