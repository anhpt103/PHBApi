using BTS.SP.API.PHB.ViewModels.PBDT.B1312;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B1312")]
    public class PHB_PBDT_B1312_Controller : ApiController
    {
        private readonly IPHB_PBDT_B1312_Service _formService;
        private readonly IPHB_PBDT_B1312_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B1312_DETAIL_Service _detailService;
        ...
        public PHB_PBDT_B1312_Controller(
            IPHB_PBDT_B1312_Service service,
            IPHB_PBDT_B1312_TEMPLATE_Service templateService,
            IPHB_PBDT_B1312_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B1312>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1312_TEMPLATE>>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1312_DETAIL>>();
            ...
            try
            {
                ...
                response.Data = _detailService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B1312_REFID == refId)
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detail.PHB_PBDT_B1312_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream)){
                ...
                for(int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B1312_DETAIL
                        {
                            STT = sheet.Cells[row, 1].Value?.ToString(),
                            CHI_TIEU = sheet.Cells[row, 2].Value?.ToString(),

                            NAMTH_SO_DOI_TUONG = decimal.Parse(sheet.Cells[row, 3].Value?.ToString() ?? "0"),
                            NAMTH_HE_SO = decimal.Parse(sheet.Cells[row, 4].Value?.ToString() ?? "0"),
                            NAMTH_KINH_PHI = decimal.Parse(sheet.Cells[row, 5].Value?.ToString() ?? "0"),

                            NAMHH_SO_DOI_TUONG = decimal.Parse(sheet.Cells[row, 6].Value?.ToString() ?? "0"),
                            NAMHH_HE_SO = decimal.Parse(sheet.Cells[row, 7].Value?.ToString() ?? "0"),
                            NAMHH_DT = decimal.Parse(sheet.Cells[row, 8].Value?.ToString() ?? "0"),
                            NAMHH_UOC_THUC_HIEN = decimal.Parse(sheet.Cells[row, 9].Value?.ToString() ?? "0"),

                            NAMKH_SO_DOI_TUONG = decimal.Parse(sheet.Cells[row, 10].Value?.ToString() ?? "0"),
                            NAMKH_HE_SO = decimal.Parse(sheet.Cells[row, 11].Value?.ToString() ?? "0"),
                            NAMKH_KINH_PHI = decimal.Parse(sheet.Cells[row, 12].Value?.ToString() ?? "0"),

                            IS_BOLD = sheet.Cells[row, 2].Style.Font.Bold ? 1 : 0,
                            IS_ITALIC = sheet.Cells[row, 2].Style.Font.Italic ? 1 : 0,
                            IS_OPTIONAL = 0,
                            PHB_PBDT_B1312_REFID = form.REFID,
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B1312();
            var lstDetail = new List<PHB_PBDT_B1312_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_B1312_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_B1312_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
