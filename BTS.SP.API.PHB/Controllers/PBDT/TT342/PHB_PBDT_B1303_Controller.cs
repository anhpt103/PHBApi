using BTS.SP.API.PHB.ViewModels.PBDT.B1303;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B1303")]
    public class PHB_PBDT_B1303_Controller : ApiController
    {
        private readonly IPHB_PBDT_B1303_Service _formService;
        private readonly IPHB_PBDT_B1303_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B1303_DETAIL_Service _detailService;
        ...
        public PHB_PBDT_B1303_Controller(
            IPHB_PBDT_B1303_Service service,
            IPHB_PBDT_B1303_TEMPLATE_Service templateService,
            IPHB_PBDT_B1303_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B1303>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1303_TEMPLATE>>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1303_DETAIL>>();
            ...
            try
            {
                ...
                response.Data = _detailService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B1303_REFID == refId)
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detail.PHB_PBDT_B1303_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream)){
                ...
                for(int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B1303_DETAIL
                        {
                            PHB_PBDT_B1303_REFID = form.REFID,
                            ...
                            //
                            CQ_CHUTRI = sheet.Cells[row, 3].Value?.ToString(),
                            THOIGIAN_THUCHIEN = sheet.Cells[row, 4].Value?.ToString(),
                            QUYETDINH_PHEDUYET = sheet.Cells[row, 5].Value?.ToString(),

                            //
                            KPPD_TONGSO = decimal.Parse(sheet.Cells[row, 6].Value?.ToString() ?? "0"),
                            KPPD_NGUON_NSNN = decimal.Parse(sheet.Cells[row, 7].Value?.ToString() ?? "0"),
                            KPPD_NGUON_KHAC = decimal.Parse(sheet.Cells[row, 8].Value?.ToString() ?? "0"),

                            //
                            NAMHH_TONGSO = decimal.Parse(sheet.Cells[row, 9].Value?.ToString() ?? "0"),
                            NAMHH_DT = decimal.Parse(sheet.Cells[row, 10].Value?.ToString() ?? "0"),
                            NAMHH_UOC_THUCHIEN = decimal.Parse(sheet.Cells[row, 11].Value?.ToString() ?? "0"),
                            NAMHH_KINHPHI_THUCHIEN = decimal.Parse(sheet.Cells[row, 12].Value?.ToString() ?? "0"),

                            //
                            LUYKE_TONGSO = decimal.Parse(sheet.Cells[row, 13].Value?.ToString() ?? "0"),
                            LUYKE_NGUON_NSNN = decimal.Parse(sheet.Cells[row, 14].Value?.ToString() ?? "0"),
                            LUYKE_NGUON_KHAC = decimal.Parse(sheet.Cells[row, 15].Value?.ToString() ?? "0"),

                            //
                            NAMKH_TONGSO = decimal.Parse(sheet.Cells[row, 16].Value?.ToString() ?? "0"),
                            NAMKH_NGUON_NSNN = decimal.Parse(sheet.Cells[row, 17].Value?.ToString() ?? "0"),
                            NAMKH_NGUON_KHAC = decimal.Parse(sheet.Cells[row, 18].Value?.ToString() ?? "0"),
                        ...
                        //check sum
                        if (CheckSum(detail) == false)
                        {
                            response.Error = true;
                            response.Message = $"Lỗi dữ liệu: Tổng số không bằng tổng các thành phần (dòng {row})";
                            return Ok(response);
                        }
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B1303();
            var lstDetail = new List<PHB_PBDT_B1303_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_B1303_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_B1303_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
        ...
        private bool CheckSum(PHB_PBDT_B1303_DETAIL detail)
        {
            if (detail.KPPD_TONGSO != detail.KPPD_NGUON_NSNN + detail.KPPD_NGUON_KHAC)
            {
                return false;
            }

            if (detail.NAMHH_TONGSO != detail.NAMHH_UOC_THUCHIEN + detail.NAMHH_KINHPHI_THUCHIEN)
            {
                return false;
            }

            if (detail.LUYKE_TONGSO != detail.LUYKE_NGUON_NSNN + detail.LUYKE_NGUON_KHAC)
            {
                return false;
            }

            if (detail.NAMKH_TONGSO != detail.NAMKH_NGUON_NSNN + detail.NAMKH_NGUON_KHAC)
            {
                return false;
            }
