using BTS.SP.API.PHB.ViewModels.PBDT.B125;
using BTS.SP.API.PHB.ViewModels.PBDT.Main;
using BTS.SP.API.PHB.ViewModels.PBDT.TT342.B125;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B125")]
    public class PHB_PBDT_B125_Controller : ApiController
    {
        private readonly IPHB_PBDT_B125_Service _B125Service;
        private readonly IPHB_PBDT_B125_DETAIL_Service _detailService;
        private readonly IPHB_PBDT_B125_TEMPLATE_Service _templateService;
        ...
        public PHB_PBDT_B125_Controller(
           IPHB_PBDT_B125_Service service,
           IPHB_PBDT_B125_DETAIL_Service detailService,
           IPHB_PBDT_B125_TEMPLATE_Service templateService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B125>();
                var lstFormViewModel = new List<FormViewModel>();

                try
                {
                    lstForms = _B125Service.Queryable().Where(x => x.NAM_HIEN_HANH == nam && x.MA_DONVI == donVi.MA_DVQHNS).ToList();
                }
                catch (Exception ex)
                {
                    response.Error = true;
        ...
        {
            var response = new Response<List<PHB_PBDT_B125_TEMPLATE>>();
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            var formCount = _B125Service.Queryable().Where(x => x.MA_DONVI == form.MA_DONVI && x.NAM_HIEN_HANH == form.NAM_HIEN_HANH).Count();
            ...
            _B125Service.Insert(form);

            foreach (var detail in model.Details)
            {
                detail.PHB_PBDT_B125_REFID = form.REFID;
        ...
        {
            var response = new Response<List<PHB_PBDT_B125_DETAIL>>();
            var form = new PHB_PBDT_B125();

            try
            {
                form = _B125Service.Queryable().FirstOrDefault(x => x.ID.Equals(id));
                response.Data = _detailService.Queryable().Where(x => x.PHB_PBDT_B125_REFID.Equals(form.REFID)).OrderBy(x => x.STT_SAPXEP).ToList();
            }
            catch (Exception ex)
            {
                response.Error = true;
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            var response = new Response();
            var form = new PHB_PBDT_B125();
            var lstDetail = new List<PHB_PBDT_B125_DETAIL>();

            try
            {

                form = _B125Service.Queryable().FirstOrDefault(x => x.ID.Equals(model.FormId));
                lstDetail = _detailService.Queryable().Where(x => x.PHB_PBDT_B125_REFID.Equals(form.REFID)).ToList();
            }
            catch (Exception ex)
            {
                response.Error = true;
            ...
            try
            {
                ...
                {
                    details.ID = new int();
                    details.PHB_PBDT_B125_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            var formCount = _B125Service.Queryable()
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream))
            {
                ...
                for (int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B125_DETAIL
                        {
                            ...
                            PHB_PBDT_B125_REFID = form.REFID,
        ...
        public IHttpActionResult Delete(int id)
        {
            ...
            {
                form = _B125Service.Queryable().FirstOrDefault(x => x.ID.Equals(id));
                lstDetail = _detailService.Queryable().Where(x => x.PHB_PBDT_B125_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                response.Error = true;
        ...
        public IHttpActionResult Approve(int formId)
        {
            ...
            try
            {
                form = _B125Service.Queryable().Where(frm => frm.ID == formId).FirstOrDefault();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
