using BTS.SP.API.PHB.ViewModels.PBDT.B1301;
...
{
    [RoutePrefix("api/phb/pbdt/tt342/PHB_PBDT_TT342_B1301")]
    public class PHB_PBDT_B1301_Controller : ApiController
    {
        private readonly IPHB_PBDT_B1301_Service _formService;
        private readonly IPHB_PBDT_B1301_TEMPLATE_Service _templateService;
        private readonly IPHB_PBDT_B1301_DETAIL_Service _detailService;
        ...
        public PHB_PBDT_B1301_Controller(
            IPHB_PBDT_B1301_Service service,
            IPHB_PBDT_B1301_TEMPLATE_Service templateService,
            IPHB_PBDT_B1301_DETAIL_Service detailService,
        ...
        public IHttpActionResult GetListForm(int nam)
        {
            ...
            {
                var lstForms = new List<PHB_PBDT_B1301>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1301_TEMPLATE>>();
        ...
        {
            var response = new Response<List<PHB_PBDT_B1301_DETAIL>>();
            ...
            try
            {
                ...
                response.Data = _detailService.Queryable()
                    .Where(detail => detail.PHB_PBDT_B1301_REFID == refId)
        ...
        public IHttpActionResult AddContent(AddContent_ViewModel model)
        {
            ...
            {
                detail.PHB_PBDT_B1301_REFID = form.REFID;
        ...
        public IHttpActionResult UploadExcel()
        {
            ...
            using (var excelPackage = new ExcelPackage(httpRequest.Files[0].InputStream)){
                ...
                for(int row = startRow; row <= endRow; row++)
                {
                    try
                    {
                        var detail = new PHB_PBDT_B1301_DETAIL
                        {
                            ...
                            IS_OPTIONAL = 0,
                            PHB_PBDT_B1301_REFID = form.REFID,
        ...
        public IHttpActionResult Edit(Edit_ViewModel model)
        {
            ...
            var form = new PHB_PBDT_B1301();
            var lstDetail = new List<PHB_PBDT_B1301_DETAIL>();

            try
            {
                form = _formService.Queryable().Where(frm => frm.ID == model.FormId).FirstOrDefault();
                lstDetail = _detailService.Queryable().Where(detail => detail.PHB_PBDT_B1301_REFID == form.REFID).ToList();
            }
            catch (Exception ex)
            {
                WriteLogs.LogError(ex);
        ...
        private bool IsOptional(List<PHB_PBDT_B1301_TEMPLATE> lstChiTieuTemplate, string chiTieu)
        {
