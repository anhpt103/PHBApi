...
namespace BTS.SP.STC.Code.FBA.Data
{
    ...
    /// </summary>
    internal class FBAUsersView : DataSourceView
    {
        private readonly Cache _cache = HttpRuntime.Cache;
        private readonly FBADataSource _owner;
        ...
        protected override IEnumerable ExecuteSelect(DataSourceSelectArguments selectArgs)
        {
            ...
            var provider = FbaSites.GetMembershipProvider(site);
            if (provider == null)
                return null;
            ...
            var cacheKey = string.Format("BTS19.QLG.App.FBAUsersView.{0}", provider);

            var spUsers = _cache.Get(cacheKey) as Dictionary<string, SPListItem>;
            ...
            if (_owner.ResetCache || spUsers == null)
            {
                ...
                var list = web
                    .SiteUserInfoList; //web.Lists[SPUtility.GetLocalizedString("$Resources:userinfo_schema_listtitle", "core", web.Language)];
                ...
                query.ViewFields =
                    "<FieldRef Name='Name' /><FieldRef Name='LinkTitle' /><FieldRef Name='Email' /><FieldRef Name='Modified' /><FieldRef Name='Created' />";
                query.RowLimit = 100000;
                //Convert SPListItemCollection to dictionary for fast lookup

                try
                {
                    var userList = list.GetItems(query);

                    if (userList != null)
                        foreach (SPListItem item in userList)
                        {
                            var username = item["Name"] as string;
                            var decodedName = FbaMembers.DecodeUsername(username);
                            if (username != decodedName) spUsers.Add(decodedName, item);
                        }
                }
                catch (Exception ex)
                {
            ...
            var totalRecords = 0;
            var spUsersCount = spUsers.Count;
            var spUsersFound = 0;

            users.BeginLoadData();

            //Add all membership users to the datatable
            foreach (MembershipUser memberuser in FbaMembers.BaseMembershipProvider(site)
                .GetAllUsers(0, 100000, out totalRecords))
            {
                string title = null;
                var email = memberuser.Email;
                DateTime? modified = null;
                DateTime? created = null;
                var isInSharepoint = no;
                ...
                if (spUsersFound < spUsersCount)
                    if (spUsers.TryGetValue(memberuser.UserName.ToLower(), out spUser))
                    {
                        ...
                        var spEmail = spUser["EMail"] as string;
                        if (!string.IsNullOrEmpty(spEmail)) email = spEmail;
                    }
            ...
            if (_owner.SearchText.Length > 0)
                dataView.RowFilter = string.Format("Name LIKE '%{0}%' OR Email LIKE '%{0}%' OR Title LIKE '%{0}%'",
                    _owner.SearchText);
            else
                dataView.RowFilter = "";
